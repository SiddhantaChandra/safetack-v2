import { RoutesModel, JourneysModel, RoutePoint, JourneyPoint, Route } from '../database/models';
 ...
 */
export const analyzeJourney = async (journeyId: number): Promise<JourneyAnalysisResult | null> => {
  try {
    // Get journey points
    ...
    let bestMatchRoute: Route | null = null;
    ...
    if (bestMatchScore >= ROUTE_SIMILARITY_THRESHOLD && bestMatchRoute && bestMatchRoute.id) {
      ...
      const newAvgDuration = bestMatchRoute.avg_duration ?
        (bestMatchRoute.avg_duration * (bestMatchRoute.times_traveled || 1) + journeyDuration) / 
        ((bestMatchRoute.times_traveled || 1) + 1) :
        journeyDuration;
 ...
 */
export const checkForDeviation = async (journeyId: number, currentPosition: Position): Promise<DeviationResult | null> => {
  try {
    // Get journey details
 ...
 */
const calculatePointSimilarity = (point1: Position, point2: Position): number => {
 ...
 */
const calculatePathSimilarity = (path1: Position[], path2: Position[]): number => {
 ...
 */
const samplePath = (path: Position[], numSamples: number): Position[] => {
  if (path.length <= numSamples) {
    return path;
  }
  
  const result: Position[] = [path[0]]; // Always include first point
 ...
 */
const simplifyRoute = <T extends Position>(points: T[], epsilon = 20): T[] => {
  if (points.length <= 2) {
    return [...points];
  }
 ...
 */
const interpolateRoutePosition = (routePoints: RoutePoint[], progress: number): Position | null => {
