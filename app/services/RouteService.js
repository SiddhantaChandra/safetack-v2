import { RoutesModel, JourneysModel } from '../database/models';
 ...
 */
export const analyzeJourney = async (journeyId) => {
  try {
    // Get journey points
    ...
    if (bestMatchScore >= ROUTE_SIMILARITY_THRESHOLD && bestMatchRoute) {
      ...
      const newConfidence = Math.min(
        bestMatchRoute.confidence_score + CONFIDENCE_INCREMENT,
 ...
 */
export const checkForDeviation = async (journeyId, currentPosition) => {
  try {
    // Get journey details
 ...
 */
const calculateRouteSimilarity = (journeyPoints, routePoints) => {
 ...
 */
const calculatePointSimilarity = (point1, point2) => {
 ...
 */
const samplePath = (path, numSamples) => {
  if (path.length <= numSamples) {
    return path;
  }
  
  const result = [path[0]]; // Always include first point
 ...
 */
const simplifyRoute = (points, epsilon = 20) => {
  if (points.length <= 2) {
    return [...points];
  }
 ...
 */
const estimateRouteProgress = (journeyPoints, routePoints) => {
 ...
 */
const interpolateRoutePosition = (routePoints, progress) => {
 ...
 */
const generateRouteName = (startLocation, endLocation) => {
